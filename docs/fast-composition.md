...
When no stable, approved concept target is available, the preview proposes `MedLearn/Inbox/<source_job_id>.md`. The current implementation uses only `StubNoteComposer`; it has no network, secret, R2, Vault, Git, clock, or random access.
