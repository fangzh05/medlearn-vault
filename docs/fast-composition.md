...
When no stable, approved concept target is available, the preview proposes `MedLearn/Inbox/<source_job_id>.md` only when `--source-job-id` was supplied. Otherwise it uses a deterministic `preview_<digest>` source-record identifier from the exact input bytes; it is not a Job ID. The default `StubNoteComposer` has no network access. Explicit `--composer deepseek` reads `DEEPSEEK_API_KEY` only from the environment, supports `deepseek-v4-flash` (default) and `deepseek-v4-pro`, and sends only the prompt, template, normalized metadata, role-separated learning material, optional current note, optional style-only golden example, and retrieved text with minimal locators. It never sends local paths, SQLite paths, retrieval queries, Git data, or unrelated files. Blockers prevent writes; warnings do not certify medical correctness; no retry, publication, Vault write, deployment, or sync occurs.

```powershell
$env:DEEPSEEK_API_KEY = "your-key"
medlearn compose preview --intake examples/intake/manual-copd.json --template templates/medical_note_v1.md --composer deepseek --prompt prompts/deepseek_note_composer_v1.md --output preview.md --json
```
