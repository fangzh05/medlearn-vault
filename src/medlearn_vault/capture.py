...
WORKFLOW_VERSION: Literal["0.1.0"] = "0.1.0"
...
class CaptureDraft(DomainModel):
    draft_version: Literal["0.1.0"] = WORKFLOW_VERSION
...
class CaptureProposal(DomainModel):
    proposal_version: Literal["0.1.0"] = WORKFLOW_VERSION
...
def _canonical(value: Any, *, exclude_proposal_digest: bool = False) -> Any:
    if isinstance(value, DomainModel):
        value = value.model_dump(mode="json")
    if isinstance(value, dict):
        return {
            key: _canonical(item, exclude_proposal_digest=exclude_proposal_digest)
...
def build_capture_proposal(bundle: ContractBundle, draft: CaptureDraft) -> CaptureProposal:
    ...
    for term in all_terms:
        ...
        evidence = tuple(sorted(set(mention.evidence_message_ids if mention else ())))
        ...
        if item.speaker_role == "user":
            ...
            observations.append(
                LearningObservationCandidate(
                    ...
                    evidence_message_ids=tuple(sorted(set(item.evidence_message_ids))),
    ...
    for misconception in draft.misconception_candidates:
        ...
        observations.append(
            LearningObservationCandidate(
                ...
                concept_refs=item_refs,
                evidence_message_ids=tuple(sorted(set(misconception.evidence_message_ids))),
...
def render_capture_proposal_markdown(proposal: CaptureProposal, *, bundle: ContractBundle) -> str:
    ...
    sections = [
        ...
        + "\n".join(
            (
                f"- source_id: `{proposal.learning_capture_candidate.source_id}`",
                f"- session_id: `{proposal.learning_capture_candidate.session_id}`",
