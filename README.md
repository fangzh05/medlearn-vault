...
在仓库根目录运行 `medlearn ui`，或在 Windows 上双击 `启动 MedLearn UI.cmd`，即可打开本地笔记生成界面。服务固定绑定 `127.0.0.1`，不会暴露到局域网。选择 DeepSeek 模式后可以选择“记住此设备”；密钥仅以当前 Windows 用户可解密的加密数据保存，页面不会显示明文，也不会写入浏览器存储、Cookie、URL 或日志。

### GPT 插件自动生成与同步

MedLearn MCP 插件提供 `generate_medical_note` 工具。GPT 将 Markdown 提交给 Worker 后，Worker 使用 Cloudflare `DEEPSEEK_API_KEY` Secret 调用 DeepSeek，并将生成笔记写入 Vault R2 的 `MedLearn/Generated/`；现有 Vault 同步会将它下载到 Obsidian。API Key 不会提供给 GPT、写入 R2、作为 API 响应返回或记入日志。

部署前，在 `worker/` 目录设置 Secret 并部署：
...
To avoid typing the preview command, start the local UI from the repository root:
...
It opens a browser workspace where you can choose the Intake JSON, Medical Note V1 template, prompt, optional local SQLite index, current note, and golden example. `stub` is the offline default; choosing DeepSeek is an explicit network opt-in. Generated Markdown is previewed in the browser and downloaded only when you click “保存 Markdown”. The service is permanently bound to `127.0.0.1`; it cannot be exposed to the LAN. A remembered UI key is encrypted for the current Windows user and is never displayed in plaintext or written to browser storage, cookies, URLs, or logs.
